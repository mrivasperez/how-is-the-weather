...
const search = (location = 'Seattle', callback) => {
    ...
    request({url: locationurl, json: true}, (error, response) => {
        ...
        if (error) {
            console.log('error');
            callback('ERROR: Unable to connect to weather service. Are you online?', undefined); 
        } else if (forecastData.error){
            console.log('error');
            callback('ERROR: Unable to find location. Please try a different search.', undefined);
        // formatted results 
        } else {
